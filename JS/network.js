...
(function () {
    ...
    /** 图表内图形整体缩放（节点位置、圆点、连线、箭头等） */
    const GRAPH_SCALE = 0.78;
    const GRAPH_CENTER = { x: 157, y: 139 };

    function scaleGraph(v, center) {
        return center + (v - center) * GRAPH_SCALE;
    }
    ...
    chart.setOption({
        ...
        graphic: [
            {
                ...
                children: [
                    { type: "line", shape: { x1: 0, y1: 0, x2: 22, y2: 0 }, style: { stroke: TIER_COLOR[1], lineWidth: 4 * GRAPH_SCALE, lineCap: "round" } },
                    { type: "text", style: { text: "一级·百万级", x: 26, y: 3, fill: "#7A7A7A", font: `${T.dense || 9}px Noto Serif SC, SimSun, serif`, textBaseline: "middle" } },
                    { type: "line", shape: { x1: 88, y1: 0, x2: 110, y2: 0 }, style: { stroke: TIER_COLOR[2], lineWidth: 2.6 * GRAPH_SCALE, lineCap: "round" } },
                    { type: "text", style: { text: "二级·数十万", x: 114, y: 3, fill: "#7A7A7A", font: `${T.dense || 9}px Noto Serif SC, SimSun, serif`, textBaseline: "middle" } },
                    { type: "line", shape: { x1: 186, y1: 0, x2: 208, y2: 0 }, style: { stroke: TIER_COLOR[3], lineWidth: 1.3 * GRAPH_SCALE, lineCap: "round" } },
        ...
        series: [{
            ...
            data: nodes.map((n) => {
                ...
                const symbolSize = Math.round(n.symbolSize * GRAPH_SCALE);
                return {
                    name: n.name,
                    x: scaleGraph(n.x, GRAPH_CENTER.x),
                    y: scaleGraph(n.y, GRAPH_CENTER.y),
                    symbolSize,
                    category: n.category,
                    fixed: true,
                    itemStyle: {
                        ...
                        borderWidth: n.category === 0 ? 1.4 : 1,
                        shadowBlur: n.category === 0 ? 6 : 4,
                    ...
                    label: {
                        ...
                        color: "#4A7C65",
                        fontSize: symbolSize >= 26 ? (T.axis || 11) : (T.axisSm || 10),
                        fontWeight: 700,
                        distance: 4,
            ...
            links: links.map((l) => ({
                ...
                lineStyle: {
                    ...
                    width: TIER_WIDTH[l.tier] * GRAPH_SCALE,
                    curveness: 0.14,
                    opacity: l.tier === 1 ? 0.88 : l.tier === 2 ? 0.72 : 0.58
                }
            })),
            edgeSymbol: ["none", "arrow"],
            edgeSymbolSize: [0, Math.round(8 * GRAPH_SCALE)],
            labelLayout,
            emphasis: {
                focus: "adjacency",
