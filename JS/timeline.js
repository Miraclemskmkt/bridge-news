...
(function () {
    ...
    function buildSvg() {
        ...
        const maxR = Math.round(22 * CHART_SCALE * INNER_SCALE);
        const tagPad = Math.round(6 * CHART_SCALE);
        const tagY = Math.round(12 * CHART_SCALE);
        const dotR = +(1.8 * INNER_SCALE).toFixed(1);
        ...
        const axisX2 = W - padR;
        svg += `<line x1="${padL}" y1="${axisY}" x2="${axisX2 - 5}" y2="${axisY}" stroke="${LINE}" stroke-width="1"/>`;
        svg += `<polygon points="${axisX2},${axisY} ${axisX2 - 5},${axisY - 2.5} ${axisX2 - 5},${axisY + 2.5}" fill="${LINE}"/>`;
        svg += `<text x="${axisX2}" y="${axisY + 13}" text-anchor="end" class="cunchao-timeline__xlabel">发展年份</text>`;
        ...
        years.forEach((item, i) => {
            ...
            svg += `<text x="${cx}" y="${tagY}" text-anchor="middle" class="cunchao-timeline__tag">${item.keyword}</text>`;

            svg += `<g class="cunchao-bubble-hit" data-index="${i}">`;
            svg += `<circle class="cunchao-bubble" cx="${cx}" cy="${cy}" r="${r}" fill="#F5F2E8" stroke="${INK}" stroke-width="0.9"/>`;
            svg += `<circle cx="${cx}" cy="${cy}" r="${dotR}" fill="${INK}" pointer-events="none"/>`;
            svg += `</g>`;
